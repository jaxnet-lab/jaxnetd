...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := jaxutil.DecodeWIF(secret)
	if err != nil {
		t.Errorf("TestFilterInsertKey DecodeWIF failed: %v", err)
		return
	}

	f := bloom.NewFilter(2, 0, 0.001, types.BloomUpdateAll)
	f.Add(wif.SerializePubKey())
	f.Add(jaxutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := jaxutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := jaxutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	f := bloom.NewFilter(10, 0, 0.000001, types.BloomUpdateP2PubkeyOnly)
