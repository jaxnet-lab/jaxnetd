...
func (cs *CoinSet) TotalValue() (value jaxutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue jaxutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue jaxutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount jaxutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue jaxutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue jaxutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue jaxutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue jaxutil.Amount, coins []Coin) (Coins, error) {
...
// jaxutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() jaxutil.Amount {
	return jaxutil.Amount(c.txOut().Value)
}
