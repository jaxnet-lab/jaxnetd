...
// PolyMod is function will compute what 8 5-bit values to XOR into the last 8 input
...
func DecodeCashAddress(str string) (string, []byte, error) {
	...
	if !VerifyChecksum(prefix, values) {
		return "", nil, jaxutil.ErrChecksumMismatch
	}
...
// DecodeBCHAddress decodes the string encoding of an address and returns
...
func DecodeBCHAddress(addr string, defaultNet *chaincfg.Params) (jaxutil.Address, error) {
	...
	if err != nil {
		if err == jaxutil.ErrChecksumMismatch {
			return nil, jaxutil.ErrChecksumMismatch
		}
	...
	case ripemd160.Size: // P2PKH or P2SH
		...
		default:
			return nil, jaxutil.ErrUnknownAddressType
		}
...
// NewCashAddressPubKeyHash returns a new CashAddressPubKeyHash.  pkHash mustbe 20
// bytes.
...
func NewCashAddressScriptHash(serializedScript []byte, net *chaincfg.Params) (*CashAddressScriptHash, error) {
	scriptHash := jaxutil.Hash160(serializedScript)
	return newCashAddressScriptHashFromHash(scriptHash, net)
}

// NewCashAddressScriptHashFromHash returns a new CashAddressScriptHash.  scriptHash
// must be 20 bytes.
...
func packAddressData(addrType AddressType, addrHash []byte) ([]byte, error) {
	...
	if addrType != P2PKH && addrType != P2SH {
		return nil, errors.New("invalid addrtype")
	}
