...
// Tx returns a wrapped transaction (jaxutil.Tx) for the transaction at the
...
// Transactions returns a slice of wrapped transactions (Tx) for all
...
// instead provides easy access to wrapped versions (Tx) of them.
...
func NewBlockFromReader(r io.Reader) (*Block, error) {
	...
	msgBlock, err := wire.DecodeBlock(r)
	if err != nil {
		return nil, err
	}
