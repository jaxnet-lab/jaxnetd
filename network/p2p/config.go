...
type Config struct {
	...
	DisableListen  bool     `yaml:"disable_listen" toml:"disable_listen" long:"nolisten" description:"Disable listening for incoming connections -- NOTE: Listening is automatically disabled if the --connect or --proxy options are used without also specifying listen interfaces via --listen"`

	ExternalIPs    []string      `yaml:"external_ips" toml:"external_ips" long:"externalip" description:"Add an ip to the list of local addresses we claim to listen on to peers"`
	...
	BanDuration    time.Duration `yaml:"ban_duration" toml:"ban_duration" long:"banduration" description:"How long to ban misbehaving peers.  Valid time units are {s, m, h}.  Minimum 1 second"`
	...
	RejectNonStd       bool          `yaml:"reject_non_std" toml:"reject_non_std" long:"rejectnonstd" description:"Reject non-standard transactions regardless of the default settings for the active network."`
