...
var ErrWebsocketsRequired = errors.New("a websocket connection is required to use this feature")
...
type notificationState struct {
	...
	notifySpent        map[jaxjson.OutPoint]struct{}
}
...
func (s *notificationState) Copy() *notificationState {
	...
	stateCopy.notifySpent = make(map[jaxjson.OutPoint]struct{})
...
func newNotificationState() *notificationState {
	return &notificationState{
		...
		notifySpent:    make(map[jaxjson.OutPoint]struct{}),
	}
}
...
type NotificationHandlers struct {
	...
	// result of a rescan request, due to how jaxnetd may send various rescan
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case jaxjson.BlockConnectedNtfnMethod:
	...
	case jaxjson.RescanFinishedNtfnMethod:
		...
		if err != nil {
			log.Warn().Msgf("Received invalid rescanfinished "+
				"notification: %v", err)
			return
		}
	...
	case jaxjson.RescanProgressNtfnMethod:
		...
		hash, height, blkTime, err := parseRescanProgressParams(ntfn.Params)
		if err != nil {
			log.Warn().Msgf("Received invalid rescanprogress "+
				"notification: %v", err)
			return
		}
	...
	case jaxjson.TxAcceptedVerboseNtfnMethod:
		...
		if err != nil {
			log.Warn().Msgf("Received invalid tx accepted verbose "+
				"notification: %v", err)
			return
		}
	...
	case jaxjson.JaxnetdConnectedNtfnMethod:
		...
		if err != nil {
			log.Warn().Msgf("Received invalid jaxnetd connected "+
				"notification: %v", err)
			return
		}
	...
	case jaxjson.AccountBalanceNtfnMethod:
		...
		if err != nil {
			log.Warn().Msgf("Received invalid account balance "+
				"notification: %v", err)
			return
		}
	...
	case jaxjson.WalletLockStateNtfnMethod:
		...
		if err != nil {
			log.Warn().Msgf("Received invalid wallet lock state "+
				"notification: %v", err)
			return
		}
...
func parseFilteredBlockConnectedParams(params []json.RawMessage, shardID uint32) (int32,
	...
	transactions := make([]*jaxutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = jaxutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
func parseChainTxNtfnParams(params []json.RawMessage) (*jaxutil.Tx,
	*jaxjson.BlockDetails, error,
	...
	// JSON object.
	var block *jaxjson.BlockDetails
	if len(params) > 1 {
	...
	return jaxutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := jaxutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	...
	// Bounds check amount.
	bal, err := jaxutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := jaxjson.NewNotifyBlocksCmd()
...
func (c *Client) notifySpentInternal(outpoints []jaxjson.OutPoint) FutureNotifySpentResult {
	...
	cmd := jaxjson.NewNotifySpentCmd(outpoints)
	return c.sendCmd(cmd)
}

// newOutPointFromWire constructs the jaxjson representation of a transaction
// outpoint from the wire type.
func newOutPointFromWire(op *wire.OutPoint) jaxjson.OutPoint {
	return jaxjson.OutPoint{
		Hash:  op.Hash.String(),
		Index: op.Index,
	}
}
...
func (c *Client) NotifySpentAsync(outpoints []*wire.OutPoint) FutureNotifySpentResult {
	...
	ops := make([]jaxjson.OutPoint, 0, len(outpoints))
	for _, outpoint := range outpoints {
		ops = append(ops, newOutPointFromWire(outpoint))
	}
	cmd := jaxjson.NewNotifySpentCmd(ops)
	return c.sendCmd(cmd)
}
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := jaxjson.NewNotifyNewTransactionsCmd(&verbose)
	return c.sendCmd(cmd)
}
...
func (c *Client) notifyReceivedInternal(addresses []string) FutureNotifyReceivedResult {
	...
	cmd := jaxjson.NewNotifyReceivedCmd(addresses)
	return c.sendCmd(cmd)
}
...
func (c *Client) NotifyReceivedAsync(addresses []jaxutil.Address) FutureNotifyReceivedResult {
	...
	cmd := jaxjson.NewNotifyReceivedCmd(addrs)
	return c.sendCmd(cmd)
}
...
//
// Deprecated: Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []jaxutil.Address) error {
...
func (c *Client) RescanAsync(startBlock *chainhash.Hash,
	...
	cmd := jaxjson.NewRescanCmd(startBlockHashStr, addrs, ops, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	...
	cmd := jaxjson.NewRescanCmd(startBlockHashStr, addrs, ops,
		&endBlockHashStr)
	return c.sendCmd(cmd)
}
...
func (c *Client) LoadTxFilterAsync(reload bool, addresses []jaxutil.Address,
	...
	outPointObjects := make([]jaxjson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = jaxjson.OutPoint{
			Hash:  outPoints[i].Hash.String(),
			Index: outPoints[i].Index,
		}
	}

	cmd := jaxjson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.sendCmd(cmd)
}
