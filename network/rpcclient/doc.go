...
In addition to the compatible standard HTTP POST JSON-RPC API, jaxnetd and
jaxwallet provide a websocket interface that is more efficient than the standard
...
jaxnetd or jaxwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by jaxnetd and
jaxwallet only uses a single connection that remains open and allows
...
All notifications provided by jaxnetd require registration to opt-in.  For example,
...
The following full-blown client examples are in the examples directory:
 ...
 - jaxnetdwebsockets
   Connects to a jaxnetd RPC server using TLS-secured websockets, registers for
   block connected and block disconnected notifications, and gets the current
   block count
 - jaxwalletwebsockets
   Connects to a jaxwallet RPC server using TLS-secured websockets, registers
