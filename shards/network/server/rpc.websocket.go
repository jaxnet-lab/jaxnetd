...
//	"errors"
//	"fmt"
//	"gitlab.com/jaxnet/core/shard.core.git"
...
//	s.logger.Infof("New websocket client %s", remoteAddr)
//	if s.ntfnMgr.NumClients()+1 > main.cfg.RPCMaxWebsockets {
//		s.logger.Infof("Max websocket clients exceeded [%d] - "+
...
//	if err != nil {
//		s.logger.Errorf("Failed to serve client %s: %v", remoteAddr, err)
//		conn.Close()
//		return
//	}
...
//	s.logger.Infof("Disconnected websocket client %s", remoteAddr)
//}
//
...
//
//			default:
//				s.logger.Warn("Unhandled notification type")
//			}
//
...
//	if err != nil {
//		s.logger.Errorf("Failed to marshal block connected notification: "+
//			"%v", err)
//		return
//	}
...
//	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
//	if err != nil {
//		s.logger.Errorf("Failed to marshal block disconnected "+
//			"notification: %v", err)
//		return
//	}
//	for _, wsc := range clients {
...
//	if err != nil {
//		s.logger.Errorf("Failed to serialize header for filtered block "+
...
//		marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
//		if err != nil {
//			s.logger.Errorf("Failed to marshal filtered block "+
//				"connected notification: %v", err)
//			return
//		}
//		wsc.QueueNotification(marshalledJSON)
...
//	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
//	if err != nil {
//		s.logger.Errorf("Failed to marshal filtered block disconnected "+
//			"notification: %v", err)
//		return
//	}
//	for _, wsc := range clients {
...
//	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
//	if err != nil {
//		s.logger.Errorf("Failed to marshal tx notification: %s", err.Error())
//		return
//	}
//
...
//				verboseNtfn)
//			if err != nil {
//				s.logger.Errorf("Failed to marshal verbose tx "+
...
//	notifyMap, ok := ops[*op]
//	if !ok {
//		s.logger.Warnf("Attempt to remove nonexistent spent request "+
...
//			marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
//			if err != nil {
//				s.logger.Errorf("Failed to marshal processedtx notification: %v", err)
//				continue
//			}
//
...
//			if err != nil {
//				s.logger.Warnf("Failed to marshal redeemingtx notification: %v", err)
//				continue
//			}
//			for wscQuit, wsc := range cmap {
//				if block != nil {
...
//	cmap, ok := addrs[addr]
//	if !ok {
//		s.logger.Warnf("Attempt to remove nonexistent addr request "+
...
//			if err != io.EOF {
//				s.logger.Errorf("Websocket receive error from "+
//					"%s: %v", c.addr, err)
//			}
//			break out
//		}
...
//			if err != nil {
//				s.logger.Errorf("Failed to marshal parse failure "+
//					"reply: %v", err)
//				continue
//			}
...
//			continue
//		}
//		s.logger.Debugf("Received command <%s> from %s", cmd.method, c.addr)
//
...
//			s.logger.Warnf("Websocket client %s is already authenticated",
//				c.addr)
//			break out
//		case !c.authenticated && !ok:
//			s.logger.Warnf("Unauthenticated websocket message " +
//				"received")
//			break out
...
//			if err != nil {
//				s.logger.Errorf("Failed to marshal authenticate reply: "+
//					"%v", err.Error())
//				continue
//			}
//			c.SendMessage(reply, nil)
...
//	c.Disconnect()
//	c.wg.Done()
//	s.logger.Tracef("Websocket client input handler done for %s", c.addr)
//}
//
...
//	if err != nil {
//		s.logger.Errorf("Failed to marshal reply for <%s> "+
...
//	}
//	c.wg.Done()
//	s.logger.Tracef("Websocket client notification queue handler done "+
//		"for %s", c.addr)
//}
//
...
//	}
//	c.wg.Done()
//	s.logger.Tracef("Websocket client output handler done for %s", c.addr)
//}
//
...
//	}
//
//	s.logger.Tracef("Disconnecting websocket client %s", c.addr)
//	close(c.quit)
//	c.conn.Close()
//	c.disconnected = true
//}
...
//	s.logger.Tracef("Starting websocket client %s", c.addr)
//
...
//				}
//				if err != nil {
//					s.logger.Errorf("Unable to notify "+
...
//				marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
//				if err != nil {
//					s.logger.Errorf("Failed to marshal recvtx notification: %v", err)
//					return
//				}
//
...
//		s.logger.Errorf("Stopping rescan for reorged block %v "+
