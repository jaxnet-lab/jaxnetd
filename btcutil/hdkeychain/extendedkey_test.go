...
func TestExtendedKeyAPI(t *testing.T) {
	...
	for i, test := range tests {
		...
		addr, err := key.Address(&chaincore.MainNetParams)
		if err != nil {
...
func TestNet(t *testing.T) {
	tests := []struct {
		name      string
		key       string
		origNet   *chaincore.Params
		newNet    *chaincore.Params
		newPriv   string
		newPub    string
		isPrivate bool
	}{
		// Private extended keys.
		{
			...
			origNet:   &chaincore.MainNetParams,
			newNet:    &chaincore.SimNetParams,
		...
		},
		{
			...
			origNet:   &chaincore.SimNetParams,
			newNet:    &chaincore.MainNetParams,
		...
		},
		{
			...
			key:       "xprv9s21ZrQH143K3QTDL4LXw2F7HEK3wJUD2nW2nRk4stbPy6cq3jPPqjiChkVvvNKmPGJxWUtg6LnF5kejMRNNU3TGtRBeJgk33yuGBxrMPHi",
			origNet:   &chaincore.MainNetParams,
			newNet:    &chaincore.RegressionNetParams,
		...
		},
		{
			...
			origNet:   &chaincore.RegressionNetParams,
			newNet:    &chaincore.MainNetParams,
			newPriv:   "xprv9s21ZrQH143K3QTDL4LXw2F7HEK3wJUD2nW2nRk4stbPy6cq3jPPqjiChkVvvNKmPGJxWUtg6LnF5kejMRNNU3TGtRBeJgk33yuGBxrMPHi",
			newPub:    "xpub661MyMwAqRbcFtXgS5sYJABqqG9YLmC4Q1Rdap9gSE8NqtwybGhePY2gZ29ESFjqJoCu1Rupje8YtGqsefD265TMg7usUDFdp6W1EGMcet8",
			isPrivate: true,
...
func TestErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			err:       nil,
			neuter:    true,
			neuterErr: chaincore.ErrUnknownHDKeyID,
		},
	}
