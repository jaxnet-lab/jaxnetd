...
PKG="gitlab.com/jaxnet/core/shard.core.git"
...
for i in $SYS; do
    ...
    env CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -trimpath -ldflags="-s -w -buildid=" gitlab.com/jaxnet/core/shard.core.git
    env CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -trimpath -ldflags="-s -w -buildid=" gitlab.com/jaxnet/core/shard.core.git/cmd/btcctl
    cd ..
