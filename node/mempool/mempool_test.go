...
func (s *fakeChain) FetchUtxoView(tx *jaxutil.Tx) (*chaindata.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *jaxutil.Tx,
	view *chaindata.UtxoViewpoint) (*chaindata.SequenceLock, error) {

	return &chaindata.SequenceLock{
		Seconds:     -1,
		BlockHeight: -1,
	}, nil
}
...
// transactions.
func txOutToSpendableOut(tx *jaxutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   jaxutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*jaxutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee jaxutil.Amount,
	signalsReplacement bool) (*jaxutil.Tx, error) {

	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput jaxutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*jaxutil.Tx, error) {
	txChain := make([]*jaxutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, jaxutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := jaxutil.NewAddressPubKey(pubKeyBytes, chainParams)
	if err != nil {
		return nil, nil, err
	}
...
// mock chain.
func (ctx *testContext) addCoinbaseTx(numOutputs uint32) *jaxutil.Tx {
	ctx.t.Helper()
...
func (ctx *testContext) addSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee jaxutil.Amount,
	signalsReplacement, confirmed bool) *jaxutil.Tx {
...
func testPoolMembership(tc *testContext, tx *jaxutil.Tx, inOrphanPool, inTxPool bool) {
	tc.t.Helper()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   jaxutil.Amount(5000000000),
...
func TestSignalsReplacement(t *testing.T) {
	...
	testCases := []struct {
		name               string
		setup              func(ctx *testContext) *jaxutil.Tx
...
func TestConflicts(t *testing.T) {
	...
	testCases := []struct {
		...
		setup func(ctx *testContext) (*jaxutil.Tx, []*jaxutil.Tx)
	}{
		{
		...
		},
		{
			...
			setup: func(ctx *testContext) (*jaxutil.Tx, []*jaxutil.Tx) {
				...
				return tx, []*jaxutil.Tx{conflict1, conflict2}
			},
		},
		{
			// Create a transaction that would introduce two
			...
			setup: func(ctx *testContext) (*jaxutil.Tx, []*jaxutil.Tx) {
				...
				return tx, []*jaxutil.Tx{parent, child}
			},
		},
	}
