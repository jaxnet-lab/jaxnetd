...
import (
	...
	"gitlab.com/jaxnet/jaxnetd/node/blockchain/fullblocktests"
...
func newBeaconBlockGen() chaindata.ChainBlockGenerator {
	...
	miner, _ := jaxutil.NewAddressPubKey(privKey.PubKey().SerializeCompressed(), &chaincfg.SimNetParams)
	return chaindata.NewBeaconBlockGen(
		...
		}, chaincfg.SimNetParams.PowParams.PowLimitBits)
}
...
func TestFullBlocks(t *testing.T) {
	...
	tests, err := fullblocktests.Generate(net, false)
	if err != nil {
		t.Fatalf("failed to generate tests: %v", err)
	}

	// Create a new database and chain instance to run tests against.
	chain, teardownFunc, err := chainSetup("fullblocktest", net)
	if err != nil {
	...
	testAcceptedBlock := func(item fullblocktests.AcceptedBlock) {
		...
		isMainChain, isOrphan, err := chain.ProcessBlock(block, chaindata.BFNone)
		if err != nil {
	...
	testRejectedBlock := func(item fullblocktests.RejectedBlock) {
		...
		_, _, err := chain.ProcessBlock(block, chaindata.BFNone)
		if err == nil {
		...
		rerr, ok := err.(chaindata.RuleError)
		if !ok {
	...
	testOrphanOrRejectedBlock := func(item fullblocktests.OrphanOrRejectedBlock) {
		...
		_, isOrphan, err := chain.ProcessBlock(block, chaindata.BFNone)
		if err != nil {
