...
func (b *BlockChain) verifyCheckpoint(height int32, hash *chainhash.Hash) bool {
	...
	// log.Info().Msgf("Verified checkpoint at height %d/block %s", checkpoint.Height,
	// 	checkpoint.Hash)
	return true
}
...
func (b *BlockChain) findPreviousCheckpoint() (blocknodes.IBlockNode, error) {
	if !b.HasCheckpoints() {
		return nil, nil
	}
...
func isNonstandardTransaction(tx *jaxutil.Tx) bool {
