...
type RejectedBlock struct {
	...
	Block      *wire.MsgBlock
	Height     int32
	RejectCode chaindata.ErrorCode
}
...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: jaxutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
type testGenerator struct {
	...
	miner   *jaxutil.AddressPubKey
}

// makeTestGenerator returns a test generator instance initialized with the
// genesis block as the tip.
func makeTestGenerator(chainCtx chainctx.IChainCtx, blockGen chaindata.ChainBlockGenerator) (testGenerator, error) {
	genesis := chainCtx.GenesisBlock()
	genesisHash := genesis.BlockHash()

	privKey, _ := btcec.PrivKeyFromBytes(btcec.S256(), []byte{0x01})
	miner, err := jaxutil.NewAddressPubKey(privKey.PubKey().SerializeCompressed(), chainCtx.Params())
	if err != nil {
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := jaxutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	if len(txns) == 1 {
		return txns[0].TxHash()
	}

	utilTxns := make([]*jaxutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, jaxutil.NewTx(tx))
	}
	merkles := chaindata.BuildMerkleTreeStore(utilTxns, false)
...
func additionalCoinbase(amount jaxutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee jaxutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func (g *testGenerator) blocksByMMMRoot(mmrRoot chainhash.Hash) *wire.MsgBlock {
	hash := g.mmrRootToBlock[mmrRoot]
	return g.blocks[hash]
}
...
func (g *testGenerator) nextBlock(blockName string, spend *spendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	header, _ := g.blockGen.NewBlockHeader(
		1,
		currentMMRRoot,
		calcMerkleRoot(txns),
		ts,
...
func (g *testGenerator) updateBlockState(oldBlockName string, oldBlockHash chainhash.Hash, newBlockName string, newBlock *wire.MsgBlock) {
	...
	g.mmr.RmBlock(oldBlockHash, blockHeight)
	// Add new entries.
...
func (g *testGenerator) saveTipCoinbaseOut() {
	g.spendableOuts = append(g.spendableOuts, makeSpendableOut(g.tip, 0, 1))
...
func newBeaconBlockGen(params *chaincfg.Params) chaindata.ChainBlockGenerator {
	privKey, _ := btcec.PrivKeyFromBytes(btcec.S256(), []byte{0x01})
	miner, _ := jaxutil.NewAddressPubKey(privKey.PubKey().SerializeCompressed(), params)
...
func Generate(params *chaincfg.Params, includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	chainCtx := chainctx.NewBeaconChain(params)
	blockGen := newBeaconBlockGen(params)

	// Create a test generator instance initialized with the genesis block
	// as the tip.
	g, err := makeTestGenerator(chainCtx, blockGen)
	if err != nil {
		return nil, err
	}
	...
	rejected := func(code chaindata.ErrorCode) {
		tests = append(tests, []TestInstance{
	...
	rejected(chaindata.ErrImmatureSpend)
	...
	rejected(chaindata.ErrBlockTooBig)
	...
	rejected(chaindata.ErrBadCoinbaseScriptLen)
	...
	rejected(chaindata.ErrNoTransactions)
	...
	// requires an unsolved block.
	{
		...
		for {
			...
			blockHash := b46.Header.PoWHash()
			hashNum := pow.HashToBig(&blockHash)
			if hashNum.Cmp(g.chainCtx.Params().PowParams.PowLimit) >= 0 {
				break
			}
		}
		g.updateBlockState("b46", origHash, "b46", b46)
	}
	rejected(chaindata.ErrHighHash)
	...
	rejected(chaindata.ErrTimeTooNew)
	...
	rejected(chaindata.ErrBadMerkleRoot)
	...
	rejected(chaindata.ErrUnexpectedDifficulty)
	...
	rejected(chaindata.ErrDuplicateTx)
	...
	rejected(chaindata.ErrTimeTooOld)
	...
	rejected(chaindata.ErrSpendTooHigh)
	...
	rejected(chaindata.ErrUnfinalizedTx)
