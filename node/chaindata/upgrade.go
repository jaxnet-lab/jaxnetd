...
func MigrateBlockIndex(db database.DB) error {
	...
	log.Info().Msgf("Block database migration complete")
	return nil
}
...
func UpgradeUtxoSetToV2(db database.DB, interrupt <-chan struct{}) error {
	...
	log.Info().Msgf("Upgrading utxo set to v2.  This will take a while...")
	start := time.Now()
	...
	for {
		...
		log.Info().Msgf("Migrated %d utxos (%d total)", numUtxos, totalUtxos)
	}
	...
	err = db.Update(func(dbTx database.Tx) error {
		...
		return RepoTx(dbTx).PutVersion(UtxoSetVersionKeyName, 2)
	})
	if err != nil {
		return err
	}
	...
	log.Info().Msgf("Done upgrading utxo set.  Total utxos: %d in %d seconds",
		totalUtxos, seconds)
	return nil
}
