...
func TestAssignField(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := jaxjson.TstAssignField(1, "testField", dst, src)
		if err != nil {
...
func TestNewCmdErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			method: "getblock",
			args:   []interface{}{},
			err:    jaxjson.Error{ErrorCode: jaxjson.ErrNumParams},
		},
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := jaxjson.NewCmd(test.method, test.args...)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
...
func TestMarshalCmdErrors(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := jaxjson.MarshalCmd(test.id, 0, test.cmd)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v), "+
				"want %T", i, test.name, err, err, test.err)
			continue
...
func TestUnmarshalCmdErrors(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := jaxjson.UnmarshalCmd(&test.request)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v), "+
				"want %T", i, test.name, err, err, test.err)
			continue
