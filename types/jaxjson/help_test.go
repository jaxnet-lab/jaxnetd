...
func TestHelpReflectInternals(t *testing.T) {
	...
	for i, test := range tests {
		...
		key := jaxjson.TstReflectTypeToJSONType(xT, test.reflectType)
		if key != test.key {
		...
		examples, isComplex := jaxjson.TstReflectTypeToJSONExample(xT,
		...
		helpText := jaxjson.TstResultTypeHelp(xT, test.reflectType, "fdk")
		...
		isValid := jaxjson.TstIsValidResultType(test.reflectType.Kind())
...
func TestResultStructHelp(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		results := jaxjson.TstResultStructHelp(xT, test.reflectType, 0)
...
func TestHelpArgInternals(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		help := jaxjson.TstArgHelp(xT, test.reflectType, test.defaults,
			test.method)
		if help != test.help {
...
func TestMethodHelp(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		help := jaxjson.TestMethodHelp(xT, test.reflectType,
...
func TestGenerateHelpErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			method: "boguscommand",
			err:    jaxjson.Error{ErrorCode: jaxjson.ErrUnregisteredMethod},
		},
		{
		...
		},
		{
			...
			err:         jaxjson.Error{ErrorCode: jaxjson.ErrMissingDescription},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := jaxjson.GenerateHelp(test.method, nil,
			test.resultTypes...)
...
func TestGenerateHelp(t *testing.T) {
	...
	help, err := jaxjson.GenerateHelp("help", descs)
	if err != nil {
