...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		in   jaxjson.UsageFlag
		want string
	}{
		{0, "0x0"},
		{jaxjson.UFWalletOnly, "UFWalletOnly"},
		{jaxjson.UFWebsocketOnly, "UFWebsocketOnly"},
		{jaxjson.UFNotification, "UFNotification"},
		{jaxjson.UFWalletOnly | jaxjson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{jaxjson.UFWalletOnly | jaxjson.UFWebsocketOnly | (1 << 31),
			"UFWalletOnly|UFWebsocketOnly|0x80000000"},
	}

	// Detect additional usage flags that don't have the stringer added.
	numUsageFlags := 0
	highestUsageFlagBit := jaxjson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			err: jaxjson.Error{ErrorCode: jaxjson.ErrDuplicateMethod},
		},
		{
			...
			flags: jaxjson.TstHighestUsageFlagBit,
			err:   jaxjson.Error{ErrorCode: jaxjson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: jaxjson.Error{ErrorCode: jaxjson.ErrEmbeddedType},
		},
		{
			...
			err: jaxjson.Error{ErrorCode: jaxjson.ErrUnexportedField},
		},
		{
			...
			err: jaxjson.Error{ErrorCode: jaxjson.ErrUnsupportedFieldType},
		},
		{
		...
		},
		{
			...
			err: jaxjson.Error{ErrorCode: jaxjson.ErrNonOptionalField},
		},
		{
			...
			err: jaxjson.Error{ErrorCode: jaxjson.ErrNonOptionalDefault},
		},
		{
			...
			err: jaxjson.Error{ErrorCode: jaxjson.ErrMismatchedDefault},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		err := jaxjson.RegisterCmd(
			jaxjson.ScopedMethod(test.scope, test.method),
			test.cmdFunc(),
			test.flags, false)
...
func TestMustRegisterCmdPanic(t *testing.T) {
	...
	jaxjson.MustRegisterCmd("", "panicme", 0, 0)
}
...
func TestRegisteredCmdMethods(t *testing.T) {
	...
	methods := jaxjson.RegisteredCmdMethods()
	if len(methods) == 0 {
