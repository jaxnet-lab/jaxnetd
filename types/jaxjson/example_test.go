...
func ExampleMarshalCmd() {
	...
	// optional fields.  Also, notice the call to jaxjson.Bool which is a
	// convenience function for creating a pointer out of a primitive for
	// optional parameters.
	blockHash := "000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f"
	gbCmd := jaxjson.NewGetBeaconBlockCmd(blockHash, jaxjson.Int(0))
	...
	id := 1
	marshalledBytes, err := jaxjson.MarshalCmd(id, 0, gbCmd)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleUnmarshalCmd() {
	...
	gbCmd, ok := cmd.(*jaxjson.GetBeaconBlockCmd)
	if !ok {
...
func ExampleMarshalResponse() {
	...
	marshalledBytes, err := jaxjson.MarshalResponse(1, 350001, nil)
	if err != nil {
		fmt.Println(err)
		return
	}
