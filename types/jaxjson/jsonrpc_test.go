...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if jaxjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *jaxjson.RPCError {
				return jaxjson.NewRPCError(jaxjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := jaxjson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := jaxjson.NewRequest(nil, jaxjson.ScopedMethod("", "test"), 0, []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := jaxjson.Error{ErrorCode: jaxjson.ErrInvalidType}
	_, err = jaxjson.MarshalResponse(make(chan int), nil, nil)
	if jerr, ok := err.(jaxjson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = jaxjson.MarshalResponse(1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			jaxjson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			jaxjson.ErrRPCMethodNotFound,
