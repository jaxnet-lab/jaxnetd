...
func signAndCheck(msg string, tx *wire.MsgTx, idx int, inputAmt int64, pkScript []byte,
	...
	sigScript, err := SignTxOutput(&chain.TestNet3Params, tx, idx,
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address, err := btcutil.NewAddressPubKeyHash(
				btcutil.Hash160(pk), &chain.TestNet3Params)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			sigScript, err := SignTxOutput(&chain.TestNet3Params,
