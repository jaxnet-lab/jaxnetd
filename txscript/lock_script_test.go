...
func TestSignTxOutput_htlc_multisig(t *testing.T) {
	// t.Parallel()
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			if !testHtlcForMultisig(t, tx.Copy(), inputAmounts, hashType, i) {
				break
			}
		}
	}
...
func testHtlcForMultisig(t *testing.T, tx *wire.MsgTx, inputAmounts []int64, hashType SigHashType, i int) bool {
	msg := fmt.Sprintf("%d:%d", hashType, i)
	...
	scriptAddr, err := jaxutil.NewAddressScriptHash(pkScript, &chaincfg.TestNet3Params)
	if err != nil {
...
func TestMultisigSanbbox(t *testing.T) {
...

	{
		...
		sigScript, err := SignTxOutput(&chaincfg.TestNet3Params, tx, i,
	...
	{
		lockPeriod := int32(24000)
		htlcScript, err := HTLCScriptAddress(pay2MultisigScriptAddr, lockPeriod, &chaincfg.TestNet3Params)
		failErr(t, err, "failed to make pkscript for %s: %v", msg, err)
