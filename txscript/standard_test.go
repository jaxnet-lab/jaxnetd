...
// newAddressPubKey returns a new jaxutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) jaxutil.Address {
	addr, err := jaxutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new jaxutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) jaxutil.Address {
	addr, err := jaxutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new jaxutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) jaxutil.Address {
	addr, err := jaxutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the jaxutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the jaxutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := jaxutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := jaxutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := jaxutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := jaxutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := jaxutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*jaxutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*jaxutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*jaxutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*jaxutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*jaxutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		script, err := MultiSigScript(test.keys, test.nrequired, false)
		if e := tstCheckScriptError(err, test.err); e != nil {
...
func TestMultiSigTimeLockScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := jaxutil.NewAddressPubKey(hexToBytes("02192d74d0cb94344c9569c2e77901573d8d7903c3ebec3a957724895dca52c6b4"),
		&chaincfg.MainNetParams)
	if err != nil {
		t.Fatalf("Unable to create pubkey address (compressed): %v",
			err)
	...
	tests := []struct {
		refundAddress         *jaxutil.AddressPubKey
		keys                  []*jaxutil.AddressPubKey
		nrequired             int
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		script, err := MultiSigLockScript(test.keys, test.nrequired, test.refundAddress, test.refundDeferringPeriod, false)
		if e := tstCheckScriptError(err, test.err); e != nil {
			t.Errorf("MultiSigLockScript #%d: %v", i, e)
			continue
		}
		scriptAddr, err := jaxutil.NewAddressScriptHash(script, &chaincfg.TestNet3Params)
		if err != nil {
