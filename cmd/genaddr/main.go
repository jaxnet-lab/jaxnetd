...
func genKeys() {
	...
	mainNetAddress, err := btcutil.NewAddressPubKeyHash(btcutil.Hash160(pk), &chain.MainNetParams)
	if err != nil {
		println("[error] " + err.Error())
		os.Exit(1)
	}

	testNetAddress, err := btcutil.NewAddressPubKeyHash(btcutil.Hash160(pk), &chain.TestNet3Params)
	if err != nil {
		println("[error] " + err.Error())
		os.Exit(1)
	}
