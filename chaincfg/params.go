...
type Params struct {
	...
	// address generation.
	HDCoinType uint32
	// ShardID defines the serial number of current ShardChain. For BC ShardID is zero.
	ShardID uint32

	// StartHeight is the height of BeaconChain block in that this ShardChain was created.
	StartHeight uint32
}

// ShardGenesis creates genesis for ShardChain based on genesis of the BeaconChain.
func (cfg Params) ShardGenesis(shard, height uint32, block *wire.MsgBlock, hash *chainhash.Hash) Params {
	// shard's exclusive info
	cfg.ShardID = shard
	cfg.StartHeight = height
	cfg.Name = cfg.Name + "_shard_" + strconv.FormatUint(uint64(shard), 16)
