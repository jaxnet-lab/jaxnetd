...
**jaxnetd** provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API with *some difference*.  There are a few key
differences between **jaxnetd** and **bitcoind**, **btcd** and **JSON-RPC** as far as how RPCs are serviced:

* Unlike **JSON-RPC** the **jaxnetd** has few additional fields:
    1. `scope` - sets the context of the request; 
        - `node` methods related to daemon/node, like **getVersion**, **ping**, etc;
        - `chain` common methods same for the `beacon` and `shard`;
        - `beacon` methods that works only for `beacon` chain;
        - `shard`  methods that works only for `shard` chains;
...
* **jaxnetd** is secure by default which means that the RPC connection is TLS-enabled by default

* **jaxnetd** provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for **jaxnetd** RPC and are used by applications
such as [**shard.wallet**](https://github.com/btcsuite/**shard.wallet**) for inter-process
communication with **jaxnetd**.  The websocket connection endpoint for **jaxnetd** is
...
during the development of **shard.wallet**.
...
The **jaxnetd** RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
**jaxnetd** is `wss://your_ip_or_domain:8334/ws`.
...
to a **jaxnetd** RPC server:

* **rpcuser** is the full-access username configured for the **jaxnetd** RPC server
* **rpcpass** is the full-access password configured for the **jaxnetd** RPC server
* **rpclimituser** is the limited username configured for the **jaxnetd** RPC server
* **rpclimitpass** is the limited password configured for the **jaxnetd** RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the **jaxnetd**
  server is configured with.  It is automatically generated by **jaxnetd** and placed
  in the **jaxnetd** home directory (which is typically `%LOCALAPPDATA%\Btcd` on
  Windows and `~/.**jaxnetd**` on POSIX-like OSes)

**NOTE:** As mentioned above, **jaxnetd** is secure by default which means the RPC
...
The **jaxnetd** RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
**jaxnetd** comes with a separate utility named `btcctl` which can be used to issue
these RPC commands via HTTP POST requests to **jaxnetd** after configuring it with the
...
| # | Scope    | Method                   | Request Type                                           | Response Type | Description |
|---|----------|--------------------------|--------------------------------------------------------|---------------|-------------|
|   | "chain"  | "addNode"                | [AddNodeCmd](AddNodeCmd)                               |               | Attempts to add or remove a persistent peer |
|   | "chain"  | "getAddedNodeInfo"       | [GetAddedNodeInfoCmd](GetAddedNodeInfoCmd)             |               | Returns information about manually added (persistent) peers. |
|   | "chain"  | "getConnectionCount"     | [GetConnectionCountCmd](GetConnectionCountCmd)         |               | Returns the number of active connections to other peers. |
|   | "chain"  | "getNetTotals"           | [GetNetTotalsCmd](GetNetTotalsCmd)                     |               | Returns a JSON object containing network traffic statistics. |
|   | "chain"  | "getPeerInfo"            | [GetPeerInfoCmd](GetPeerInfoCmd)                       |               | Returns information about each connected network peer as an array of json objects. |
|   | "chain"  | "node"                   | [NodeCmd](NodeCmd)                                     |               | Queues a ping to be sent to each connected peer. |
|   | "chain"  | "ping"                   | [PingCmd](PingCmd)                                     |               | Returns a JSON object with information about the provided hex-encoded script. |
|   | "chain"  | "decodeScript"           | [DecodeScriptCmd](DecodeScriptCmd)                     |               | Verifies the given address is valid.  NOTE: Since **jaxnetd** does not have a wallet integrated, **jaxnetd** will only return whether the address is valid or not. |
|   | "chain"  | "validateAddress"        | [ValidateAddressCmd](ValidateAddressCmd)               |               | Verifies the block chain database. |
|   | "chain"  | "verifyChain"            | [VerifyChainCmd](VerifyChainCmd)                       |               |             |
|   | "chain"  | "verifyMessage"          | [VerifyMessageCmd](VerifyMessageCmd)                   |               |             |
|   | "chain"  | "getCurrentNet"          | [GetCurrentNetCmd](GetCurrentNetCmd)                   |               |             |
|   | "chain"  | "createRawTransaction"   | [CreateRawTransactionCmd](CreateRawTransactionCmd)     |               | Returns a new transaction spending the provided inputs and sending to the provided addresses. |
|   | "chain"  | "decodeRawTransaction"   | [DecodeRawTransactionCmd](DecodeRawTransactionCmd)     |               | Returns a JSON object representing the provided serialized, hex-encoded transaction. |
|   | "chain"  | "estimateFee"            | [EstimateFeeCmd](EstimateFeeCmd)                       |               |             |
|   | "chain"  | "estimateSmartFee"       | [EstimateSmartFeeCmd](EstimateSmartFeeCmd)             |               |             |
|   | "chain"  | "getMempoolInfo"         | [GetMempoolInfoCmd](GetMempoolInfoCmd)                 |               | Returns a JSON object containing mempool-related information. |
|   | "chain"  | "getRawMempool"          | [GetRawMempoolCmd](GetRawMempoolCmd)                   |               | Returns an array of hashes for all of the transactions currently in the memory pool. |
|   | "chain"  | "getRawTransaction"      | [GetRawTransactionCmd](GetRawTransactionCmd)           |               | Returns information about a transaction given its hash. |
|   | "chain"  | "getTxOut"               | [GetTxOutCmd](GetTxOutCmd)                             |               |             |
|   | "chain"  | "listTxOut"              | [ListTxOutCmd](ListTxOutCmd)                           |               |             |
|   | "chain"  | "searchRawTransactions"  | [SearchRawTransactionsCmd](SearchRawTransactionsCmd)   |               |             |
|   | "chain"  | "sendRawTransaction"     | [SendRawTransactionCmd](SendRawTransactionCmd)         |               |  Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">**jaxnetd** does not yet implement the `allowhighfees` parameter, so it has no effect</font> |
|   | "chain"  | "getBestBlock"           | [GetBestBlockCmd](GetBestBlockCmd)                     |               |             |
|   | "chain"  | "getBestBlockHash"       | [GetBestBlockHashCmd](GetBestBlockHashCmd)             |               | Returns the hash of the of the best (most recent) block in the longest block chain. |
|   | "chain"  | "getBlockchainInfo"      | [GetBlockChainInfoCmd](GetBlockChainInfoCmd)           |               |             |
|   | "chain"  | "getBlockCount"          | [GetBlockCountCmd](GetBlockCountCmd)                   |               | Returns the number of blocks in the longest block chain. |
|   | "chain"  | "getBlockHash"           | [GetBlockHashCmd](GetBlockHashCmd)                     |               | Returns hash of the block in best block chain at the given height. |
|   | "chain"  | "getCFilter"             | [GetCFilterCmd](GetCFilterCmd)                         |               |             |
|   | "chain"  | "getCFilterHeader"       | [GetCFilterHeaderCmd](GetCFilterHeaderCmd)             |               |             |
|   | "chain"  | "submitBlock"            | [SubmitBlockCmd](SubmitBlockCmd)                       |               | Attempts to submit a new serialized, hex-encoded block to the network. |
|   | "node"   | "version"                | [VersionCmd](VersionCmd)                               |               |             |
|   | "node"   | "getInfo"                | [GetInfoCmd](GetInfoCmd)                               |               |             |
|   | "node"   | "getNetworkInfo"         | [GetNetworkInfoCmd](GetNetworkInfoCmd)                 |               |             |
|   | "node"   | "uptime"                 | [UptimeCmd](UptimeCmd)                                 |               |             |
|   | "node"   | "manageShards"           | [ManageShardsCmd](ManageShardsCmd)                     |               |             |
|   | "node"   | "listShards"             | [ListShardsCmd](ListShardsCmd)                         |               |             |
|   | "node"   | "generate"               | [GenerateCmd](GenerateCmd)                             |               |             |
|   | "node"   | "getDifficulty"          | [GetDifficultyCmd](GetDifficultyCmd)                   |               |             |
|   | "node"   | "getMiningInfo"          | [GetMiningInfoCmd](GetMiningInfoCmd)                   |               |             |
|   | "node"   | "getNetworkHashPs"       | [GetNetworkHashPSCmd](GetNetworkHashPSCmd)             |               |             |
|   | "node"   | "getBlockStats"          | [GetBlockStatsCmd](GetBlockStatsCmd)                   |               |             |
|   | "node"   | "getChainTxStats"        | [GetChainStatsCmd](GetChainStatsCmd)                   |               |             |
|   | "node"   | "debugLevel"             | [DebugLevelCmd](DebugLevelCmd)                         |               |             |
|   | "node"   | "help"                   | [HelpCmd](HelpCmd)                                     |               |             |
|   | "node"   | "stop"                   | [StopCmd](StopCmd)                                     |               |             |
|   | "beacon" | "getBeaconBlock"         | [GetBeaconBlockCmd](GetBeaconBlockCmd)                 |               |             |
|   | "beacon" | "getBeaconBlockHeader"   | [GetBeaconBlockHeaderCmd](GetBeaconBlockHeaderCmd)     |               |             |
|   | "beacon" | "getBlockHeader"         | [GetBeaconBlockHeaderCmd](GetBeaconBlockHeaderCmd)     |               |             |
|   | "beacon" | "getBeaconBlockTemplate" | [GetBeaconBlockTemplateCmd](GetBeaconBlockTemplateCmd) |               |             |
|   | "beacon" | "getBeaconHeaders"       | [GetBeaconHeadersCmd](GetBeaconHeadersCmd)             |               |             |
|   | "shard"  | "getShardBlock"          | [GetShardBlockCmd](GetShardBlockCmd)                   |               |             |
|   | "shard"  | "getShardBlockHeader"    | [GetShardBlockHeaderCmd](GetShardBlockHeaderCmd)       |               |             |
|   | "shard"  | "getShardBlockTemplate"  | [GetShardBlockTemplateCmd](GetShardBlockTemplateCmd)   |               |             |
|   | "shard"  | "getShardHeaders"        | [GetShardHeadersCmd](GetShardHeadersCmd)               |               |             |
...
|---|---|
|Method|getblock|
|Parameters|1. block hash (string, required) - the hash of the block<br />2. verbose (boolean, optional, default=true) - specifies the block is returned as a JSON object instead of hex-encoded string<br />3. verbosetx (boolean, optional, default=false) - specifies that each transaction is returned as a JSON object and only applies if the `verbose` flag is true.<font color="orange">**This parameter is a **jaxnetd** extension**</font>|
...
|Notes|NOTE: Since **jaxnetd** does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in **shard.wallet** for a version which includes that information.|
...
|Notes|<font color="orange">Since **jaxnetd** does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
...
|Notes|NOTE: Since **jaxnetd** does not have the wallet integrated to provide payment addresses, **jaxnetd** must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">**jaxnetd** does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown **jaxnetd**.|
|Returns|`"**jaxnetd** stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For **jaxnetd** this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">Btcd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for **jaxnetd**.</font>|
...
The following is an overview of the RPC methods which are implemented by **jaxnetd**, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network **jaxnetd** is running on.|None|
...
|Parameters|None|
|Description|Get bitcoin network **jaxnetd** is running on.|
|Returns|numeric|
...
|Method|version|
|Parameters|None|
|Description|Returns the version of the JSON-RPC API built into this release of **jaxnetd**.|
|Returns|`{ (json object)`<br />&nbsp;&nbsp;`"**jaxnetd**jsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "x.y.z",  (string) the version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": x,  (numeric) the major version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": y,  (numeric) the minor version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": z,  (numeric) the patch version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",  (string) prerelease info for the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""  (string) metadata about the server build`<br />&nbsp;&nbsp;`}`<br />`}`|
|Example Return|`{`<br />&nbsp;&nbsp;`"**jaxnetd**jsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "1.0.0",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": 1,  `<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""`<br />&nbsp;&nbsp;`}`<br />`}`|
[Return to Overview](#MethodOverview)<br />
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this **jaxnetd** instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
**jaxnetd** uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll **jaxnetd** for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[rpcclient](https://gitlab.com/jaxnet/jaxnetd/tree/master/rpcclient) package.
...
[rpcclient](https://gitlab.com/jaxnet/jaxnetd/tree/master/rpcclient) package to connect with
a **jaxnetd** instance via Websockets, issues [getblockcount](#getblockcount) to
...
import (
	...
	"gitlab.com/jaxnet/jaxnetd/rpcclient"
	"gitlab.com/jaxnet/jaxnetd/btcutil"
)

func main() {
	// Load the certificate for the TLS connection which is automatically
	// generated by **jaxnetd** when it starts the RPC server and doesn't already
	// have one.
	**jaxnetd**HomeDir := btcutil.AppDataDir("**jaxnetd**", false)
	certs, err := ioutil.ReadFile(filepath.Join(**jaxnetd**HomeDir, "rpc.cert"))
	if err != nil {
		log.Fatal(err)
	}
...
The following is an example Go application which uses the
[rpcclient](https://gitlab.com/jaxnet/jaxnetd/tree/master/rpcclient) package to connect with
a **jaxnetd** instance via Websockets, issues [getblock](#getblock) to retrieve
...
import (
	...
	"gitlab.com/jaxnet/jaxnetd/chaincfg/chainhash"
	"gitlab.com/jaxnet/jaxnetd/rpcclient"
	"gitlab.com/jaxnet/jaxnetd/btcutil"
)
...
The following is an example Go application which uses the
[rpcclient](https://gitlab.com/jaxnet/jaxnetd/tree/master/rpcclient) package to connect with
a **jaxnetd** instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a **jaxnetd** instance,
...
// generated by **jaxnetd** when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/**jaxnetd**/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The **jaxnetd** generated certificate acts as
