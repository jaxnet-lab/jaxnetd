...
**shard.core** provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API with *some difference*.  There are a few key
differences between **shard.core** and **bitcoind**, **btcd** and **JSON-RPC** as far as how RPCs are serviced:

* Unlike **JSON-RPC** the **shard.core** has few additional fields:
...
* **shard.core** is secure by default which means that the RPC connection is TLS-enabled by default

* **shard.core** provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for **shard.core** RPC and are used by applications
such as [**shard.wallet**](https://github.com/btcsuite/**shard.wallet**) for inter-process
communication with **shard.core**.  The websocket connection endpoint for **shard.core** is
...
The **shard.core** RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
**shard.core** is `wss://your_ip_or_domain:8334/ws`.
...
to a **shard.core** RPC server:

* **rpcuser** is the full-access username configured for the **shard.core** RPC server
* **rpcpass** is the full-access password configured for the **shard.core** RPC server
* **rpclimituser** is the limited username configured for the **shard.core** RPC server
* **rpclimitpass** is the limited password configured for the **shard.core** RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the **shard.core**
  server is configured with.  It is automatically generated by **shard.core** and placed
  in the **shard.core** home directory (which is typically `%LOCALAPPDATA%\Btcd` on
  Windows and `~/.**shard.core**` on POSIX-like OSes)

**NOTE:** As mentioned above, **shard.core** is secure by default which means the RPC
...
The **shard.core** RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
**shard.core** comes with a separate utility named `btcctl` which can be used to issue
these RPC commands via HTTP POST requests to **shard.core** after configuring it with the
...
|   | "chain"  | "decodeScript"           | [DecodeScriptCmd](DecodeScriptCmd)                     |               | Verifies the given address is valid.  NOTE: Since **shard.core** does not have a wallet integrated, **shard.core** will only return whether the address is valid or not. |
...
|   | "chain"  | "sendRawTransaction"     | [SendRawTransactionCmd](SendRawTransactionCmd)         |               |  Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">**shard.core** does not yet implement the `allowhighfees` parameter, so it has no effect</font> |
...
|---|---|
|Method|getblock|
|Parameters|1. block hash (string, required) - the hash of the block<br />2. verbose (boolean, optional, default=true) - specifies the block is returned as a JSON object instead of hex-encoded string<br />3. verbosetx (boolean, optional, default=false) - specifies that each transaction is returned as a JSON object and only applies if the `verbose` flag is true.<font color="orange">**This parameter is a **shard.core** extension**</font>|
...
|Notes|NOTE: Since **shard.core** does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in **shard.wallet** for a version which includes that information.|
...
|Notes|<font color="orange">Since **shard.core** does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
...
|Notes|NOTE: Since **shard.core** does not have the wallet integrated to provide payment addresses, **shard.core** must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">**shard.core** does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown **shard.core**.|
|Returns|`"**shard.core** stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
The following is an overview of the RPC methods which are implemented by **shard.core**, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network **shard.core** is running on.|None|
...
|Parameters|None|
|Description|Get bitcoin network **shard.core** is running on.|
|Returns|numeric|
...
|Method|version|
|Parameters|None|
|Description|Returns the version of the JSON-RPC API built into this release of **shard.core**.|
|Returns|`{ (json object)`<br />&nbsp;&nbsp;`"**shard.core**jsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "x.y.z",  (string) the version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": x,  (numeric) the major version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": y,  (numeric) the minor version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": z,  (numeric) the patch version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",  (string) prerelease info for the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""  (string) metadata about the server build`<br />&nbsp;&nbsp;`}`<br />`}`|
|Example Return|`{`<br />&nbsp;&nbsp;`"**shard.core**jsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "1.0.0",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": 1,  `<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""`<br />&nbsp;&nbsp;`}`<br />`}`|
[Return to Overview](#MethodOverview)<br />
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this **shard.core** instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
**shard.core** uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll **shard.core** for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
a **shard.core** instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by **shard.core** when it starts the RPC server and doesn't already
	// have one.
	**shard.core**HomeDir := btcutil.AppDataDir("**shard.core**", false)
	certs, err := ioutil.ReadFile(filepath.Join(**shard.core**HomeDir, "rpc.cert"))
	if err != nil {
		log.Fatal(err)
	}
...
The following is an example Go application which uses the
[rpcclient](https://gitlab.com/jaxnet/core/shard.core/tree/master/rpcclient) package to connect with
a **shard.core** instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://gitlab.com/jaxnet/core/shard.core/tree/master/rpcclient) package to connect with
a **shard.core** instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a **shard.core** instance,
...
// generated by **shard.core** when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/**shard.core**/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The **shard.core** generated certificate acts as
