...
func TestHeadersWire(t *testing.T) {
	...
	bh := shard.NewBlockHeader(blockOne.Header.Version(), hash, merkleHash, mmrHash,
		blockOne.Header.Timestamp(), bits, nonce)
...
func TestHeadersWireErrors(t *testing.T) {
	...
	timestamp := blockOne.Header.Timestamp()

	bits := uint32(0x1d00ffff)
	nonce := uint32(0x9962e301)
	bh := shard.NewBlockHeader(version, hash, merkleHash, mmrHash, timestamp, bits, nonce)
	...
	// to force errors.
	bhTrans := shard.NewBlockHeader(version, hash, merkleHash, mmrHash, timestamp, bits, nonce)
